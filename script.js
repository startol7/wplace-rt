/***** Guard: surface any runtime error as on-screen toast *****/
...
let map;
let territories = new Map();           // key -> ownerCode
...
/***** Color auto-assign (distinct-ish per country) *****/
...
function updateStats(){
  ...
  const max=200000; // 仮想上限（超細かいグリッド用）
...
function updateLeaderboard(){
  ...
  box.innerHTML = top.map(([code,count],i)=>{
    const d=countryData[code]; const rk=i<3?`rank-${i+1}`:'';
    return `<div class="leaderboard-item">
      <span class="rank ${rk}">${i+1}</span>
      <span class="country-info"><span>${d?.flag||''}</span><span>${d?.name||code}</span></span>
...
function updateHeatmap(){
  ...
  const AGG=0.02; // 粗め集計（超細かいグリッドの負荷軽減）
...
document.addEventListener('DOMContentLoaded', ()=>{
  try{
    populateCountrySelect();                         // 1) セレクト先に生成
    document.getElementById('countrySelect').addEventListener('change', (e)=>{
      currentCountry = e.target.value; updateStats(); showNotification(`Changed to ${countryData[currentCountry]?.name||currentCountry}`,'info');
    });
    if(initMap()){                                   // 2) その後に地図初期化
      initializeGame();
    ...
    console.error(err); showNotification('Startup error','error');
  }
});
